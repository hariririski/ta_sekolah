<?php
   class M_edit_siswa extends CI_Model{    
        function process($id){
					...
					$siswa=$this->db->update('siswa',$siswa);
					
					$this->db->where('id_ayah',$id);
					$ayah=$this->db->update('ayah',$ayah); 
					
					$this->db->where('id_ibu',$id);
					$ibu=$this->db->update('ibu',$ibu); 
					
					$this->db->where('id_wali',$id);
					$ibu=$this->db->update('wali',$wali); 					
